...
const TMP_V3A = new Float32Array(3);
const TMP_Q4A = new Float32Array(4);
...
function assertVec3(name, v) {
    if (!v || v.length < 3)
        throw new TypeError(`${name} must be a Vec3`);
    const [x, y, z] = v;
...
export function mat4Perspective(out, fovy, aspect, near, far) {
    ...
    if (!(fovy < Math.PI))
        throw new RangeError('fovy must be < PI radians');
    // WebGPU 0..1 depth range
    m4.perspectiveZO(out, fovy, aspect, near, far);
    return out;
}
/**
...
export function mat4Ortho(out, left, right, bottom, top, near, far) {
    ...
    if (!(far > near))
        throw new RangeError('far must be greater than near');
    m4.orthoZO(out, left, right, bottom, top, near, far);
    return out;
}
/**
...
export function mat4LookAt(out, eye, target, up) {
    ...
    if (!(upLen > 0))
        throw new RangeError('up vector must be non-zero');
 ...
 */
export function mat4FromRotationTranslation(out, angle, translation) {
    assertMat4('out', out);
    assertFinite('angle', angle);
    assertVec3('translation', translation);
    const qq = glmQuat.create();
    glmQuat.setAxisAngle(qq, [0, 1, 0], angle);
    m4.fromRotationTranslation(out, qq, translation);
    return out;
}
/**
 ...
 * @returns The same `out` matrix containing the transformation values.
 */
export function mat4FromQuatTranslation(out, rotation, translation) {
    assertMat4('out', out);
    assertQuat('rotation', rotation);
    assertVec3('translation', translation);
    m4.fromRotationTranslation(out, rotation, translation);
    return out;
}
/**
 ...
 * @returns The same `out` matrix containing the transformation values.
 */
export function mat4FromQuatTranslationScale(out, rotation, translation, scale) {
    assertMat4('out', out);
    assertQuat('rotation', rotation);
    assertVec3('translation', translation);
    ...
    m4.fromRotationTranslationScale(out, rotation, translation, scale);
    return out;
}
/**
 ...
 * @returns The same `out` matrix
 */
export function mat4Translate(out, translation) {
    assertMat4('out', out);
    assertVec3('translation', translation);
    m4.fromTranslation(out, translation);
    return out;
}
/**
 ...
 * @returns The same `out` matrix
 */
export function mat4Scale(out, scale) {
    assertMat4('out', out);
    assertVec3('scale', scale);
    m4.fromScaling(out, scale);
    return out;
}
/**
 ...
 * @returns The same `out` matrix
 */
export function mat4Rotate(out, axis, angle) {
    assertMat4('out', out);
    assertVec3('axis', axis);
    assertFinite('angle', angle);
    const unit = normalizeVec3(axis);
    m4.fromRotation(out, angle, unit);
    return out;
}
/**
 ...
 */
export function mat4GetTranslation(a) {
    assertMat4('a', a);
 ...
 */
export function mat4GetTranslationOut(out, a) {
    assertVec3('out', out);
    assertMat4('a', a);
    out[0] = a[12];
    out[1] = a[13];
 ...
 */
export function mat4GetScale(a) {
    assertMat4('a', a);
 ...
 */
export function mat4GetScaleOut(out, a) {
    assertVec3('out', out);
    assertMat4('a', a);
 ...
 */
export function mat4GetRotation(a) {
    assertMat4('a', a);
 ...
 */
export function mat4GetRotationOut(out, a) {
    assertQuat('out', out);
    assertMat4('a', a);
    const sx = Math.hypot(a[0], a[1], a[2]);
    const sy = Math.hypot(a[4], a[5], a[6]);
 ...
 */
export function mat4Lerp(out, a, b, t) {
    assertMat4('out', out);
    assertMat4('a', a);
    assertMat4('b', b);
    assertFinite('t', t);
 ...
 */
export function normalizeVec3Out(out, vec) {
    assertVec3('vec', vec);
    assertVec3('out', out);
...
export function normalizeVec3Like(vec) {
    return normalizeVec3([vec[0], vec[1], vec[2]]);
}
// ========== Quaternion utilities ==========
function assertQuat(name, q) {
    if (!q || q.length < 4)
        throw new TypeError(`${name} must be a Quat`);
    const [x, y, z, w] = q;
 ...
 */
export function quatNormalizeOut(out, q) {
    assertQuat('q', q);
    assertQuat('out', out);
    const len = Math.hypot(q[0], q[1], q[2], q[3]);
    if (!(len > 0))
        throw new RangeError('quatNormalize: zero-length quaternion');
...
export function quatMultiplyOut(out, a, b) {
    assertQuat('out', out);
    assertQuat('a', a);
    assertQuat('b', b);
    glmQuat.multiply(out, a, b);
    return out;
}
/**
...
export function quatFromAxisAngleOut(out, axis, angle) {
    assertQuat('out', out);
    assertVec3('axis', axis);
    assertFinite('angle', angle);
    const unit = normalizeVec3(axis);
    glmQuat.setAxisAngle(out, unit, angle);
    return out;
}
/**
...
export function quatFromEulerOut(out, euler) {
    assertQuat('out', out);
 ...
 */
export function transformVec3ByQuat(vec, quat) {
    transformVec3ByQuatOut(TMP_V3A, vec, quat);
    return [TMP_V3A[0], TMP_V3A[1], TMP_V3A[2]];
}
export function transformVec3ByQuatOut(out, vec, quat) {
    assertVec3('out', out);
    assertVec3('vec', vec);
...
export function addVec3Out(out, a, b) {
    assertVec3('out', out);
    assertVec3('a', a);
    assertVec3('b', b);
    out[0] = a[0] + b[0];
...
export function subVec3Out(out, a, b) {
    assertVec3('out', out);
    assertVec3('a', a);
    assertVec3('b', b);
    out[0] = a[0] - b[0];
...
export function scaleVec3Out(out, vec, scalar) {
    assertVec3('out', out);
    assertVec3('vec', vec);
...
export function crossVec3Out(out, a, b) {
    assertVec3('out', out);
    assertVec3('a', a);
    assertVec3('b', b);
 ...
 * @returns The length
 */
export function lengthVec3(vec) {
    assertVec3('vec', vec);
 ...
 */
export function clampVec3(vec, minVal, maxVal) {
    assertVec3('vec', vec);
 ...
 */
export function vec3Equals(a, b, epsilon = 1e-6) {
    assertVec3('a', a);
    assertVec3('b', b);
 ...
 */
export function lengthSquaredVec3(vec) {
    assertVec3('vec', vec);
 ...
 */
export function distanceSquaredVec3(a, b) {
    assertVec3('a', a);
    assertVec3('b', b);
    const dx = b[0] - a[0];
    const dy = b[1] - a[1];
...
export function lerpVec3Out(out, a, b, t) {
    assertVec3('out', out);
    assertVec3('a', a);
    assertVec3('b', b);
    assertFinite('t', t);
...
export function quatInverseOut(out, q) {
    assertQuat('out', out);
    assertQuat('q', q);
    glmQuat.invert(out, q);
    return out;
}
/**
...
export function quatSlerpOut(out, a, b, t) {
    assertQuat('out', out);
    assertQuat('a', a);
    assertQuat('b', b);
    assertFinite('t', t);
    glmQuat.slerp(out, a, b, t);
    return out;
}
