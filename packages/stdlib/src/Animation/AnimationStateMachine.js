...
export class AnimationStateMachine {
        ...
        for (const [name, rawValue] of Object.entries(values)) {
            ...
            switch (definition.type) {
                ...
                case 'number': {
                    ...
                    this.parameterValues.set(name, Number.isFinite(numeric)
                        ? numeric
    ...
    compileState(state) {
        const transitions = (state.transitions ?? []).map((transition) => this.compileTransition(transition));
        return {
            name: state.name,
    ...
    compileTransition(transition) {
        ...
        const consumeTriggers = Array.from(new Set(conditions
    ...
    evaluateTransitionConditions(conditions) {
            ...
            switch (condition.operator) {
                ...
                case '<=': {
                    ...
                    const compareValue = typeof condition.value === 'number'
                        ...
                        : Number(condition.value);
                    const numericValue = typeof value === 'number' ? value : Number(value);
                    if (!Number.isFinite(compareValue) || !Number.isFinite(numericValue))
                        return false;
                    switch (condition.operator) {
                        case '>':
                            if (!(numericValue > compareValue))
    ...
    ensureParameterDefinition(name, type) {
        ...
        if (existing.type !== type) {
            throw new TypeError(`AnimationStateMachine: parameter "${name}" already defined as ${existing.type}, cannot redefine as ${type}`);
        }
    }
    consumeTriggerParameters(names) {
        if (!names.length)
            return;
    ...
    sanitizeBlendDuration(duration) {
        if (!Number.isFinite(duration))
            return 0;
        return Math.max(0, duration ?? 0);
    }
    startTransition(target, blendDuration, consumeTriggers) {
        if (this.currentState === target)
            return;
        if (this.blendState && this.blendState.target === target)
            return;
