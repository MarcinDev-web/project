...
export class LightingUniformWriter {
  ...
  writeLightingUniforms(
    ...
    const pointLights = lightingData.lights.filter((l: PackedLight) => l.type === 1).slice(0, 4);
    u32[f++] = pointLights.length as unknown as number; // occupies first 4 bytes
    f32[f++] = 0; // pad
    f32[f++] = 0; // pad
    f32[f++] = 0; // pad

    // Directional light (dir + color), with sane defaults if absent
    const dirLight = lightingData.lights.find((l: PackedLight) => l.type === 0) ?? {
      type: 0,
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (light) {
        // type (as u32) + 3 pad
        u32[f++] = light.type as unknown as number;
        f32[f++] = 0;
        f32[f++] = 0;
        f32[f++] = 0;
        // position (vec3) + pad
 ...
 * - 192-448: pointLights array (4 * 64 bytes)
 */
export function writeLightingUniforms(
  device: GPUDevice,
  uniformBuffer: GPUBuffer,
  lightingData: LightingData
): void {
