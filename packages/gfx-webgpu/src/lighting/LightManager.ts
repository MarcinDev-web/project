...
export class LightManager {
  private cachedLightingData: LightingData | null = null;
   ...
   */
  private packLight(entity: Entity, lightComp: LightComponent): PackedLight {
    const worldPos = entity.transform.getWorldPosition();

    // Normalize direction
    const dir = [...lightComp.direction] as Vec3;
   ...
   */
  public getLightingData(frameId: number): LightingData {
    ...
    const packedLights: PackedLight[] = [];
    const ambientColor: Vec3 = [0, 0, 0];
    ...
    for (const entity of lights) {
      ...
      if (lightComp.lightType === 'directional' && dirCount >= MAX_DIRECTIONAL_LIGHTS) continue;
      if (lightComp.lightType === 'point' && pointCount >= MAX_POINT_LIGHTS) continue;
      if (lightComp.lightType === 'spot' && spotCount >= MAX_SPOT_LIGHTS) continue;

      const packed = this.packLight(entity, lightComp);
      packedLights.push(packed);

      // Increment counters
      if (lightComp.lightType === 'directional') dirCount++;
      if (lightComp.lightType === 'point') pointCount++;
      if (lightComp.lightType === 'spot') spotCount++;

      // Stop if we've hit the max total lights
      if (packedLights.length >= MAX_LIGHTS) break;
    }
    ...
    if (packedLights.length === 0 && ambientIntensity <= 0) {
      ...
      packedLights.push({
        ...
        direction: [0.3, -0.7, -0.5] as Vec3,
        color: [1.0, 0.98, 0.95] as Vec3,
        range: 0,
        spotInnerCos: 0,
        spotOuterCos: 0,
      });
   ...
   */
  public static createDefaultLights(scene: Scene): void {
