...
export class TextureManager {
    ...
    constructor(proceduralTextureSize = 128) {
     ...
     */
    async loadTexture(id, faceTexture) {
     ...
     */
    async loadTextureFromUrl(id, faceTexture) {
        // Load albedo texture
     ...
     */
    generateProceduralTexture(id, faceTexture) {
        ...
        const albedo = this.imageDataToLoadedTexture(`${id}_albedo`, pbrTextures.albedo);
        ...
        if (normal)
            globalTextureCache.add(normal.id, normal.data, normal.width, normal.height);
        if (roughness)
            globalTextureCache.add(roughness.id, roughness.data, roughness.width, roughness.height);
        if (metallic)
            globalTextureCache.add(metallic.id, metallic.data, metallic.width, metallic.height);
        if (ao)
            globalTextureCache.add(ao.id, ao.data, ao.width, ao.height);
        return {
            id,
            albedo,
            ...(normal ? { normal } : {}),
     ...
     */
    imageDataToLoadedTexture(id, imageData) {
        return {
            id,
     ...
     */
    async loadBatch(textures) {
        const promises = textures.map(({ id, faceTexture }) => this.loadTexture(id, faceTexture));
    ...
    unloadTexture(id) {
        ...
        if (texture.normal)
            globalTextureCache.release(texture.normal.id);
        if (texture.roughness)
            globalTextureCache.release(texture.roughness.id);
        if (texture.metallic)
            globalTextureCache.release(texture.metallic.id);
        if (texture.ao)
            globalTextureCache.release(texture.ao.id);
     ...
     */
    createFallbackTexture(id, color = [1, 0, 1, 1] // Magenta for visibility
    ) {
        const size = 4;
     ...
     */
    async preloadBlockTextures(blockId, textures) {
