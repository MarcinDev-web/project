...
export class ConnectedTextureSystem {
     ...
     */
    static getTextureIndex(face, neighbors, config) {
     ...
     */
    static getHorizontalIndex(face, neighbors) {
     ...
     */
    static getVerticalIndex(neighbors) {
     ...
     */
    static getCrossIndex(face, neighbors) {
     ...
     */
    static getPillarIndex(face, neighbors) {
        const hasTop = neighbors.top;
        const hasBottom = neighbors.bottom;
        if (face === 'top') {
            // Top face of block
     ...
     */
    static getRandomIndex(variants) {
     ...
     */
    static getRandomIndexByPosition(position, variants) {
        // Simple hash based on position
        const hash = (Math.abs(position[0] * 374761393) +
     ...
     */
    static hasNeighborLeft(face, neighbors) {
        switch (face) {
            case 'north':
     ...
     */
    static hasNeighborRight(face, neighbors) {
        switch (face) {
            case 'north':
                return neighbors.east;
            case 'south':
     ...
     */
    static getNeighbors(position, scene) {
     ...
     */
    static shouldConnect(blockA, blockB, config) {
...
export class CTMTextureMapper {
     ...
     */
    static getUVs(textureIndex, gridWidth, gridHeight) {
     ...
     */
    static getCrossAtlasIndex(neighbors) {
...
export class CTMDebugger {
     ...
     */
    static visualizeNeighbors(neighbors) {
        return `
    ${neighbors.top ? '▲' : '○'}
  ...
  N:${neighbors.north ? '✓' : '✗'} S:${neighbors.south ? '✓' : '✗'}
     ...
     */
    static describeTextureIndex(index, pattern) {
        const parts = [];
...
//# sourceMappingURL=ConnectedTextures.js.map