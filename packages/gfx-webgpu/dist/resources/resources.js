...
export function validateGeometryData(geometry) {
    ...
    const dv = new DataView(geometry.vertices.buffer, geometry.vertices.byteOffset, geometry.vertices.byteLength);
 ...
 */
function packVerticesFloat32ToPacked20(source) {
...
function buildSubdividedCube(segmentsPerEdge) {
 ...
 */
export function createTimestampResources(device, supportsTimestampQueries, counts) {
...
export function createGeometryBuffers(device, geometry) {
    ...
    device.queue.writeBuffer(vertexBuffer, 0, geometry.vertices.buffer, geometry.vertices.byteOffset, geometry.vertices.byteLength);
    const indexBuffer = device.createBuffer({
        label: 'cube-index-buffer',
        size: geometry.indices.byteLength,
        usage: GPUBufferUsage.INDEX | GPUBufferUsage.COPY_DST,
    });
    device.queue.writeBuffer(indexBuffer, 0, geometry.indices.buffer, geometry.indices.byteOffset, geometry.indices.byteLength);
    const instanceOffsetBuffer = device.createBuffer({
        label: 'instance-offset-buffer',
        size: geometry.instanceOffsetData.byteLength,
        usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST,
    });
    device.queue.writeBuffer(instanceOffsetBuffer, 0, geometry.instanceOffsetData.buffer, geometry.instanceOffsetData.byteOffset, geometry.instanceOffsetData.byteLength);
    ...
    device.queue.writeBuffer(instanceRotationBuffer, 0, geometry.instanceRotationData.buffer, geometry.instanceRotationData.byteOffset, geometry.instanceRotationData.byteLength);
    // NEW: Material ID buffer for texture atlas
    const expectedMaterialIdsBytes = Math.max(geometry.instanceMaterialIdData?.byteLength ?? 0, Math.max(0, geometry.instanceCount) * 4);
    ...
    if (geometry.instanceMaterialIdData && geometry.instanceMaterialIdData.byteLength > 0) {
        device.queue.writeBuffer(instanceMaterialIdBuffer, 0, geometry.instanceMaterialIdData.buffer, geometry.instanceMaterialIdData.byteOffset, geometry.instanceMaterialIdData.byteLength);
    }
 ...
 */
export function createUniformResources(device, options) {
 ...
 */
export function createTextureResources(device, textureBindGroupLayout, textureSize = 128) {
    const layout = textureBindGroupLayout ??
 ...
 */
export function createTextureAtlas(device, _textureBindGroupLayout, atlasSize = 2048, materialTextureSize = 128) {
    ...
    if (atlas.getConfig().generateMipmaps && atlasMipmaps.length > 0) {
        for (let mipLevel = 0; mipLevel < atlasMipmaps.length; mipLevel++) {
            ...
            device.queue.writeTexture({ texture: atlasTexture, mipLevel }, mipData, { bytesPerRow: mipSize * 4, rowsPerImage: mipSize }, [mipSize, mipSize, 1]);
        }
        for (let mipLevel = 0; mipLevel < normalMipmaps.length; mipLevel++) {
            const mipData = normalMipmaps[mipLevel];
            const mipSize = Math.max(1, atlasSize >> mipLevel);
            device.queue.writeTexture({ texture: normalAtlasTexture, mipLevel }, mipData, { bytesPerRow: mipSize * 4, rowsPerImage: mipSize }, [mipSize, mipSize, 1]);
        }
    }
    else {
        // Fallback to single level
        device.queue.writeTexture({ texture: atlasTexture }, atlasData, { bytesPerRow: atlasSize * 4, rowsPerImage: atlasSize }, [atlasSize, atlasSize, 1]);
        device.queue.writeTexture({ texture: normalAtlasTexture }, normalAtlasData, { bytesPerRow: atlasSize * 4, rowsPerImage: atlasSize }, [atlasSize, atlasSize, 1]);
    }
...
const deviceIdToShaderModule = new WeakMap();
/**
 ...
 */
export function createPipelines(device, colorFormat, uniformBindGroupLayout, textureBindGroupLayout, vertexBuffers, options) {
    ...
    const shaderModuleAny = shaderModule;
    return Promise.resolve()
        .then(() => (typeof shaderModuleAny.getCompilationInfo === 'function' ? shaderModuleAny.getCompilationInfo() : { messages: [] }))
 ...
 */
export function createDepthTexture(device, canvasElement, sampleCount) {
    return createRenderAttachmentTexture(device, canvasElement, 'depth24plus', sampleCount, 'frame-depth-texture');
}
/**
 ...
 */
export function createMsaaColorTarget(device, canvasElement, format, sampleCount) {
    return createRenderAttachmentTexture(device, canvasElement, format, sampleCount, 'frame-msaa-color-texture');
}
/**
 * Creates an HDR color target (single-sampled) used as resolve target for the main pass.
 */
export function createHdrColorTarget(device, canvasElement) {
 ...
 */
export function createRenderAttachmentTexture(device, canvasElement, format, sampleCount, label) {
    return device.createTexture({
        label,
        size: { width: canvasElement.width, height: canvasElement.height, depthOrArrayLayers: 1 },
        format,
 ...
 */
export function createTextureFromData(device, width, height, data, label) {
    ...
    const queueAny = device.queue;
    if (typeof queueAny.writeTexture === 'function') {
        queueAny.writeTexture({ texture }, data.buffer, { bytesPerRow: width * 4 }, { width, height });
    }
    else {
        ...
        queueAny.writeBuffer(stagingBuffer, 0, padded.buffer);
        const encoder = device.createCommandEncoder({ label: `${label}-copy-encoder` });
        const anyEncoder = encoder;
        if (typeof anyEncoder.copyBufferToTexture === 'function') {
            anyEncoder.copyBufferToTexture({ buffer: stagingBuffer, bytesPerRow: paddedBytesPerRow }, { texture }, { width, height });
        }
        else {
        ...
        queueAny.submit([anyEncoder.finish()]);
    }
    return texture;
}
