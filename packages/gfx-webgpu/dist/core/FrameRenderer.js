...
export class FrameRenderer {
    ...
    renderFrame(ctx, viewProjectionMatrix, eyePosition, passDescriptor, viewMatrix, projectionMatrix) {
        ...
        if (this.depthTextureSize.width !== canvas.width || this.depthTextureSize.height !== canvas.height) {
            ...
            frameResources.msaaColorTexture = createMsaaColorTarget(device, canvas, 'rgba16float', MSAA_SAMPLE_COUNT);
        ...
        if (frameResources.timestampQuerySet) {
            try {
                encoder.writeTimestamp?.(frameResources.timestampQuerySet, 0);
            }
            catch {
        ...
        if (this.bundleDirty || !this.staticBundle) {
            try {
                this.staticBundle = this.recordStaticBundle(device, frameResources, ctx.presentationFormat, geometry);
        ...
        if (frameResources.timestampQuerySet) {
            try {
                encoder.writeTimestamp?.(frameResources.timestampQuerySet, 1);
            }
            catch {
                // ignore when not supported by mock
            }
    ...
    recordStaticBundle(device, frameResources, presentationFormat, geometry) {
