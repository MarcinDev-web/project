...
export class UniformManager {
    ...
    // Shadow & camera extra
    viewMatrix;
    constructor(device, uniformBuffer) {
        this.device = device;
    ...
    initializeStaticUniforms(atlasConfig) {
        ...
        this.device.queue.writeBuffer(this.uniformBuffer, 80, this.atlasInsetAndPad.buffer, this.atlasInsetAndPad.byteOffset, 16);
        this.device.queue.writeBuffer(this.uniformBuffer, 96, this.shadingParams0.buffer, this.shadingParams0.byteOffset, 16);
        ...
        this.device.queue.writeBuffer(this.uniformBuffer, 112, this.atlasParams.buffer, this.atlasParams.byteOffset, 16);
    }
    /**
     * Updates dynamic uniform data (called per frame).
     */
    updateDynamicUniforms(viewProjectionMatrix, eyePosition, lightingData) {
        // Update view-projection matrix
        this.device.queue.writeBuffer(this.uniformBuffer, 0, viewProjectionMatrix.buffer, viewProjectionMatrix.byteOffset, 64);
        // Update camera position
        this.eyePosition[0] = eyePosition[0];
        this.eyePosition[1] = eyePosition[1];
        this.eyePosition[2] = eyePosition[2];
        this.eyePosition[3] = 0;
        this.device.queue.writeBuffer(this.uniformBuffer, 64, this.eyePosition.buffer, this.eyePosition.byteOffset, 16);
    ...
    updateShadowUniforms(params) {
        ...
        // viewMatrix
        this.device.queue.writeBuffer(this.uniformBuffer, baseOffset, params.viewMatrix.buffer, params.viewMatrix.byteOffset ?? 0, 64);
        // lightViewProj[4]
        for (let i = 0; i < 4; i++) {
            const m = params.lightViewProj[i] ?? new Float32Array(16);
            this.device.queue.writeBuffer(this.uniformBuffer, baseOffset + 64 + i * 64, m.buffer, m.byteOffset ?? 0, 64);
        }
        // cascadeSplits
...
//# sourceMappingURL=UniformManager.js.map