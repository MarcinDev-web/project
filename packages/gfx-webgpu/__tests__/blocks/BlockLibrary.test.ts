...
describe('BlockLibrary', () => {
  ...
  describe('getBlocksByCategory', () => {
    ...
    it('should return blocks from gameplay category (includes glass and lights)', () => {
      const gameplay = getBlocksByCategory('gameplay');
      expect(gameplay.length).toBeGreaterThanOrEqual(2); // light_white, glass_clear

      for (const block of gameplay) {
        expect(block.category).toBe('gameplay');
      }
      
      // Verify glass and light are in gameplay
      const ids = gameplay.map(b => b.id);
      expect(ids).toContain('glass_clear');
      expect(ids).toContain('light_white');
    });
  ...
  describe('getAllCategories', () => {
    ...
    it('should return exactly 3 categories', () => {
      const categories = getAllCategories();
  ...
  describe('Block Material Properties', () => {
    ...
    it('should have glass block with low roughness', () => {
      const glass = getBlock('glass_clear');
      expect(glass?.properties.roughness).toBeLessThan(0.2);
      expect(glass?.properties.transparent).toBe(true);
    });
  ...
  describe('Block Texture Patterns', () => {
    ...
    it('should have appropriate pattern for grass block', () => {
      const grass = getBlock('grass');
