...
describe('Build Integration Tests', () => {
  ...
  describe('package exports', () => {
    ...
    it('should export specific component from @engine/world/components/*', async () => {
      const { CheckpointComponent } = await import('@engine/world/components/CheckpointComponent');
      expect(CheckpointComponent).toBeDefined();
    });
  });

  describe('file timestamps', () => {
    it('should have recent build artifacts', () => {
      ...
      if (existsSync(indexJs) && existsSync(indexDts)) {
        ...
        const dtsStat = statSync(indexDts);
        
        // Files should exist and have reasonable timestamps
        // Use 7 days tolerance to account for CI/CD environments where builds might be older
        // In local development, files are usually fresh; in CI they might be from previous runs
        // Just verify files are not extremely old (older than 30 days suggests missing build)
        const oneMonthAgo = Date.now() - (30 * 24 * 60 * 60 * 1000);
        expect(jsStat.mtimeMs).toBeGreaterThan(oneMonthAgo);
        expect(dtsStat.mtimeMs).toBeGreaterThan(oneMonthAgo);
        
        // Files should exist (we already checked existsSync, but verify they have content)
        expect(jsStat.size).toBeGreaterThan(0);
        expect(dtsStat.size).toBeGreaterThan(0);
      } else {
        // If files don't exist, skip this test (build may not have been run)
