...
import { MicroBlockMesher } from '../../src/utils/MicroBlockMesher';
import { MicroBlockStore } from '@engine/microblocks';
...
describe('MicroBlockMesher', () => {
  ...
  it('should dispose resources', () => {
    // Check if dispose exists (might be interface check)
    if ('dispose' in mesher) {
      expect(() => (mesher as any).dispose()).not.toThrow();
    }
  });
});
