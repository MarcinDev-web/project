...
export class CharacterController extends Component {
    ...
    setInput(input) {
        ...
        if (input.cameraForward && input.cameraRight) {
            this.getCameraRelativeDirectionOut(this.moveInput, input.moveDirection, input.cameraForward, input.cameraRight);
        }
    ...
    updateState() {
        if (this.isGrounded) {
            const horizontalSpeed = Math.sqrt(this.velocity[0] * this.velocity[0] +
     ...
     */
    autoRotateToMovement() {
        const entity = this.entity;
        if (!entity)
            return;
        // Only rotate if moving
        if (this.moveInput[0] === 0 && this.moveInput[2] === 0)
            return;
     ...
     */
    getCameraRelativeDirectionOut(out, input, cameraForward, cameraRight) {
        // Project camera forward onto horizontal plane
        const forward = [cameraForward[0], 0, cameraForward[2]];
        const right = [cameraRight[0], 0, cameraRight[2]];
        // Normalize
        const forwardNorm = [0, 0, 0];
        const rightNorm = [0, 0, 0];
