...
class OctreeNode {
    ...
    /** Parent node */
    parent;
    constructor(bounds, depth, parent = null) {
        this.bounds = bounds;
        this.depth = depth;
        this.parent = parent;
    }
    ...
    split() {
        ...
        this.children = [
            ...
            new OctreeNode({ min: [min[0], min[1], min[2]], max: [center[0], center[1], center[2]] }, this.depth + 1, this), // 0: ---
            new OctreeNode({ min: [center[0], min[1], min[2]], max: [max[0], center[1], center[2]] }, this.depth + 1, this), // 1: +--
            new OctreeNode({ min: [min[0], min[1], center[2]], max: [center[0], center[1], max[2]] }, this.depth + 1, this), // 2: --+
            new OctreeNode({ min: [center[0], min[1], center[2]], max: [max[0], center[1], max[2]] }, this.depth + 1, this), // 3: +-+
            // Top octants (y = center to max)
            new OctreeNode({ min: [min[0], center[1], min[2]], max: [center[0], max[1], center[2]] }, this.depth + 1, this), // 4: -+-
            new OctreeNode({ min: [center[0], center[1], min[2]], max: [max[0], max[1], center[2]] }, this.depth + 1, this), // 5: ++-
            new OctreeNode({ min: [min[0], center[1], center[2]], max: [center[0], max[1], max[2]] }, this.depth + 1, this), // 6: -++
            new OctreeNode({ min: [center[0], center[1], center[2]], max: [max[0], max[1], max[2]] }, this.depth + 1, this), // 7: +++
        ];
 ...
 */
export const DEFAULT_OCTREE_CONFIG = {
...
export class Octree {
    ...
    insertIntoNode(node, entry) {
        ...
        if (node.isLeaf()) {
            ...
            const shouldSplit = node.entities.length > this.config.maxEntitiesPerNode &&
    ...
    removeFromNode(node, entity) {
        ...
        if (node.children) {
            ...
            const totalEntities = node.children.reduce((sum, child) => sum + (child.isLeaf() ? child.entities.length : 0), 0);
     ...
     */
    collectPairsFromNode(node, pairs, checked) {
        if (node.isLeaf()) {
