...
export class CollisionDetection {
     ...
     */
    static detectCollision(colliderA, transformA, colliderB, transformB) {
        ...
        if (colliderA.shape === 'box' && colliderB.shape === 'box') {
            return this.boxBox(colliderA, posA, transformA.rotation, transformA.scale, colliderB, posB, transformB.rotation, transformB.scale);
        }
        else if (colliderA.shape === 'sphere' && colliderB.shape === 'sphere') {
            return this.sphereSphere(colliderA, posA, transformA.scale, colliderB, posB, transformB.scale);
        }
        else if (colliderA.shape === 'box' && colliderB.shape === 'sphere') {
            return this.boxSphere(colliderA, posA, transformA.rotation, transformA.scale, colliderB, posB, transformB.scale);
        }
        else if (colliderA.shape === 'sphere' && colliderB.shape === 'box') {
            const result = this.boxSphere(colliderB, posB, transformB.rotation, transformB.scale, colliderA, posA, transformA.scale);
     ...
     */
    static getWorldPosition(center, transform) {
     ...
     */
    static boxBox(boxA, posA, rotA, scaleA, boxB, posB, rotB, scaleB) {
     ...
     */
    static sphereSphere(sphereA, posA, scaleA, sphereB, posB, scaleB) {
     ...
     */
    static boxSphere(box, boxPos, boxRot, boxScale, sphere, spherePos, sphereScale) {
     ...
     */
    static capsuleCollision(colliderA, transformA, colliderB, transformB) {
        ...
        const normal = dist > this.EPSILON ? [dx / dist, dy / dist, dz / dist] : [0, 1, 0];
    ...
    static getCapsuleSegmentOrPoint(collider, t) {
        if (collider.shape === 'capsule') {
            ...
            const localA = [0, -half, 0];
            const localB = [0, half, 0];
    ...
    static getEffectiveRadius(collider, t) {
        if (collider.shape === 'capsule') {
    ...
    static closestPointsBetweenSegments(A, B, C, D) {
        const EPS = this.EPSILON;
        const u = [B[0] - A[0], B[1] - A[1], B[2] - A[2]];
        const v = [D[0] - C[0], D[1] - C[1], D[2] - C[2]];
        const w0 = [A[0] - C[0], A[1] - C[1], A[2] - C[2]];
        ...
        const pA = [A[0] + sc * u[0], A[1] + sc * u[1], A[2] + sc * u[2]];
        const pB = [C[0] + tc * v[0], C[1] + tc * v[1], C[2] + tc * v[2]];
        return { pA, pB };
    }
    static addVec(a, b) {
     ...
     */
    static boxToOBB(box, position, rotation, scale) {
        const matrix = quatToMatrix3(rotation);
        const a0 = [matrix[0], matrix[3], matrix[6]];
        const a1 = [matrix[1], matrix[4], matrix[7]];
        const a2 = [matrix[2], matrix[5], matrix[8]];
    ...
    static obbIntersect(a, b) {
        ...
        const t = [this.dot(tWorld, Au), this.dot(tWorld, Av), this.dot(tWorld, Aw)];
     ...
     */
    static inverseRotateVector(v, q) {
