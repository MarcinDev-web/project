import { addVec3Out, subVec3Out, scaleVec3Out, dotVec3, transformVec3ByQuatOut, } from '@engine/core/math';
...
export class PhysicsRaycast {
     ...
     */
    static rayBoxIntersection(ray, boxCollider, entityPosition, entityRotation, entityScale) {
        ...
        for (let i = 0; i < 3; i++) {
            if (Math.abs(localDir[i]) < this.EPSILON) {
                ...
                let normal1 = i === 0 ? [-1, 0, 0] : i === 1 ? [0, -1, 0] : [0, 0, -1];
                let normal2 = i === 0 ? [1, 0, 0] : i === 1 ? [0, 1, 0] : [0, 0, 1];
                if (t1 > t2) {
                    [t1, t2] = [t2, t1];
        ...
        const localPointTMP = [0, 0, 0];
        const localPoint = addVec3Out(localPointTMP, localOrigin, scaleVec3Out([0, 0, 0], localDir, distance));
     ...
     */
    static raySphereIntersection(ray, sphereCollider, entityPosition, entityScale) {
     ...
     */
    static rayCapsuleIntersection(ray, capsuleCollider, entityPosition, entityRotation, entityScale) {
        ...
        const p1 = [0, -halfHeight, 0];
        const p2 = [0, halfHeight, 0];
     ...
     */
    static raycastEntity(ray, entity, hitTriggers = false) {
        ...
        for (let i = 0; i < physics.colliders.length; i++) {
            ...
            switch (collider.shape) {
                case 'box':
                    result = this.rayBoxIntersection(ray, collider, entity.transform.position, entity.transform.rotation, entity.transform.scale);
                    break;
                case 'sphere':
                    result = this.raySphereIntersection(ray, collider, entity.transform.position, entity.transform.scale);
                    break;
                case 'capsule':
                    result = this.rayCapsuleIntersection(ray, collider, entity.transform.position, entity.transform.rotation, entity.transform.scale);
                    break;
            }
     ...
     */
    static worldToLocal(point, position, rotation) {
        const translated = [0, 0, 0];
     ...
     */
    static localToWorld(point, position, rotation) {
