...
import { Octree, DEFAULT_OCTREE_CONFIG } from './Octree';
...
export class PhysicsSystem {
    ...
    triggerExitListeners = [];
    /** Track previous frame's overlapping triggers */
    previousTriggers = new Set();
    ...
    constructor(scene, config = {}) {
        ...
        if (this.config.useSpatialPartitioning && this.config.worldBounds) {
            this.octree = new Octree(this.config.worldBounds, this.config.octreeConfig ?? DEFAULT_OCTREE_CONFIG);
        }
    }
    /**
     ...
     */
    integrateVelocities(entity, physics, dt) {
        const transform = entity.transform;
        // Update position (use setter to mark transform dirty)
        const pos = transform.position;
        if (!physics.freezePositionX)
            pos[0] += physics.velocity[0] * dt;
        if (!physics.freezePositionY)
            pos[1] += physics.velocity[1] * dt;
        if (!physics.freezePositionZ)
            pos[2] += physics.velocity[2] * dt;
        transform.position = pos;
        // Update rotation from angular velocity
        // Convert angular velocity to axis-angle
        const angularSpeed = Math.sqrt(physics.angularVelocity[0] ** 2 +
        ...
        if (angularSpeed > 0.0001) {
            ...
            // Apply constraints
            if (physics.freezeRotationX)
                axis[0] = 0;
            if (physics.freezeRotationY)
                axis[1] = 0;
            if (physics.freezeRotationZ)
                axis[2] = 0;
    ...
    detectCollisions(entities) {
        ...
        for (const [entityA, entityB] of pairs) {
            ...
            if (!physicsA || !physicsB)
                continue;
            if (physicsA.colliders.length === 0 || physicsB.colliders.length === 0)
                continue;
            ...
            for (const colliderA of physicsA.colliders) {
                for (const colliderB of physicsB.colliders) {
                    ...
                    const result = CollisionDetection.detectCollision(colliderA, transformA, colliderB, transformB);
     ...
     */
    getBroadPhasePairsBruteForce(entities) {
        const pairs = [];
    ...
    resolveCollision(collision) {
        ...
        if (totalInvMass === 0)
            return; // Both are static/kinematic
        ...
        if (physicsA.rigidbodyType === RigidbodyType.Dynamic) {
            ...
            if (!physicsA.freezePositionX)
                posA[0] -= normal[0] * correctionA;
            if (!physicsA.freezePositionY)
                posA[1] -= normal[1] * correctionA;
            if (!physicsA.freezePositionZ)
                posA[2] -= normal[2] * correctionA;
            entityA.transform.position = posA;
        }
        if (physicsB.rigidbodyType === RigidbodyType.Dynamic) {
            const posB = entityB.transform.position;
            if (!physicsB.freezePositionX)
                posB[0] += normal[0] * correctionB;
            if (!physicsB.freezePositionY)
                posB[1] += normal[1] * correctionB;
            if (!physicsB.freezePositionZ)
                posB[2] += normal[2] * correctionB;
     ...
     */
    applyFriction(collision, totalInvMass, invMassA, invMassB) {
