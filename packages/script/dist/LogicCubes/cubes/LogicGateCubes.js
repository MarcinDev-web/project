...
export class ANDGate extends LogicCube {
    ...
    onSignalReceived(portId, signal) {
        ...
        const inputA = this.getState('inputA', false);
        const inputB = this.getState('inputB', false);
        if (inputA && inputB) {
            const resetAfterOutput = this.getConfig('resetAfterOutput', true);
            if (resetAfterOutput) {
                this.setState('inputA', false);
                this.setState('inputB', false);
            }
...
export class NOTGate extends LogicCube {
    ...
    onSignalReceived(portId, signal) {
        if (portId === 'input') {
            ...
            const triggered = this.getState('triggered', false);
...
export class DelayGate extends LogicCube {
    ...
    onSignalReceived(portId, signal) {
        if (portId === 'input') {
            const delay = this.getConfig('delay', 1);
     ...
     */
    checkAndConsumeOutput() {
        const shouldOutput = this.getState('shouldOutput', false);
        if (shouldOutput) {
...
//# sourceMappingURL=LogicGateCubes.js.map