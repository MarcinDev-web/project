...
export class CompareVariableCondition extends LogicCube {
    ...
    onSignalReceived(portId, signal) {
        ...
        const operator = this.getConfig('operator', 'equal');
        const compareValueStr = this.getConfig('compareValue', '0');
        // Get variable value (from state for now)
        const currentValue = this.getState(`var_${variableName}`, 0);
        // Parse compare value
        let compareValue;
...
export class IsPlayerNearCondition extends LogicCube {
    ...
    onSignalReceived(portId, signal) {
        if (portId !== 'trigger')
            return null;
        const radius = this.getConfig('radius', 5);
...
export class CheckDistanceCondition extends LogicCube {
    ...
    onSignalReceived(portId, signal) {
        if (portId !== 'trigger')
            return null;
        const distance = this.getConfig('distance', 10);
        const operator = this.getConfig('operator', 'lessThan');
        const playerDetection = this.getPlayerDetection();
        let result = false;
        if (playerDetection) {
            const position = this.entity.transform.position;
...
//# sourceMappingURL=ConditionCubes.js.map