...
export async function handleHandshake(hello: HandshakeHello, ctx: HandshakeContext): Promise<HandshakeAccept | null> {
  if (hello.protocolVersion !== 1) return null;
  const token = hello.capabilities ? (hello as any).zoneToken as string | undefined : undefined;
