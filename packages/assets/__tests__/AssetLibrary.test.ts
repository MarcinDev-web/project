...
describe('AssetLibrary', () => {
    ...
    it('should convert blocks to assets', () => {
      const mockBlockLibrary = {
        test_block: {
          ...
          category: 'basic',
          material: 'plastic',
          textures: { top: { color: [1, 0, 0, 1] as [number, number, number, number] } }
        }
      };
      const assets = convertBlocksToAssets(mockBlockLibrary);
    ...
    it('should convert block with correct properties', () => {
      const mockBlockLibrary = {
        test_block: {
          ...
          category: 'natural',
          material: 'stone',
          textures: { top: { color: [0.5, 0.5, 0.5, 1] as [number, number, number, number] } }
        }
      };
      const assets = convertBlocksToAssets(mockBlockLibrary);
      const testBlock = assets.find((a) => a.metadata.id === 'block_test_block');

      expect(testBlock).toBeDefined();
      expect(testBlock?.metadata.name).toBe('Test Block');
      expect(testBlock?.category).toBe('Nature');
      expect(testBlock?.material).toBe('Stone');
      expect(testBlock?.isPlaceable).toBe(true);
      expect(testBlock?.tags).toContain('block');
    });
    ...
    it('should set built-in flag for converted blocks', () => {
      const mockBlockLibrary = {
        test_block: {
          ...
          category: 'basic',
          material: 'wood',
          textures: { top: { color: [1, 1, 1, 1] as [number, number, number, number] } }
        }
      };
      const assets = convertBlocksToAssets(mockBlockLibrary);
  ...
  describe('Asset Types Distribution', () => {
    it('should have multiple asset types', () => {
      ...
      const assets = getAllBuiltInAssets(mockBlockLibrary);
