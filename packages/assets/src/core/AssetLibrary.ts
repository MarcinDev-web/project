...
import type { Asset, AssetCollection, AssetVariant, AssetSubcategory, AssetMaterial, AssetStyle, BlockDefinition } from './AssetTypes';
 ...
 */
export function convertBlocksToAssets(blockLibrary: Record<string, BlockDefinition & { id?: string; name: string; category: string; material: string; textures: { top: { color: [number, number, number, number] } } }>): Asset[] {
  const assets: Asset[] = [];

  for (const [blockId, block] of Object.entries(blockLibrary)) {
    const asset: Asset = {
      type: 'block',
 ...
 */
export function getAllBuiltInAssets(blockLibrary?: Record<string, BlockDefinition & { id?: string; name: string; category: string; material: string; textures: { top: { color: [number, number, number, number] } } }>): Asset[] {
  const blockAssets = blockLibrary ? convertBlocksToAssets(blockLibrary) : [];
...
export async function initializeAssetLibrary(
  ...
  blockLibrary?: Record<string, BlockDefinition & { id?: string; name: string; category: string; material: string; textures: { top: { color: [number, number, number, number] } } }>
): Promise<void> {
  console.debug('Initializing Asset Library V2...');

  // Register all assets
  const assets = getAllBuiltInAssets(blockLibrary);
  ...
  console.debug(`Initialized ${assets.length} assets and ${collections.length} collections`);
}
