export interface WasmMeshProcessor {
   ...
   * 
   * Optimized for terrain sculpting - only recalculates normals for the
   * affected region + 1-ring neighborhood instead of the entire mesh.
   * 
   * @param positions - All vertex positions (x, y, z) for the entire mesh
   * @param indices - All triangle indices (u16)
   * @param normals - Existing normals buffer (modified in-place)
...
export async function init(): Promise<WasmMeshProcessor> {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
  const compute_normals_region_u16 = mod.compute_normals_region_u16 as (
    positions: Float32Array,
    indices: Uint16Array,
    normals: Float32Array,
    resolution: number,
  ...
  return {
    ...
    computeNormalsRegionU16: (positions, indices, normals, resolution, minX, maxX, minZ, maxZ) => {
      compute_normals_region_u16(positions, indices, normals, resolution, minX, maxX, minZ, maxZ);
    },
