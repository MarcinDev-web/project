 ...
 */
export function getTimestampPeriod(device: GPUDevice, adapter: GPUAdapter): number {
  let determined = false;
  let timestampPeriod = 1;

  const queueInfo = device.queue as unknown as {
  ...
  if (!determined) {
    const adapterInfo = adapter as unknown as { limits?: { timestampPeriod?: number } };
