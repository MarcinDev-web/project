...
export class VariableData extends LogicCube {
  ...
  override onInit(): void {
    const initialValue = this.getConfig<string>('initialValue', '0');
...
export class CounterData extends LogicCube {
  ...
  override onInit(): void {
    const initialValue = this.getConfig<number>('initialValue', 0);
    this.setState('count', initialValue);
  }

  onSignalReceived(portId: string, signal: LogicSignal): Map<string, LogicSignal> | null {
    const step = this.getConfig<number>('step', 1);
    let count = this.getState<number>('count', 0);
    ...
    if (portId === 'increment') {
      ...
      count = this.getConfig<number>('initialValue', 0);
      this.setState('count', count);
    } else {
      return null;
    }
...
export class TimerData extends LogicCube {
  ...
  override onInit(): void {
    const autoStart = this.getConfig<boolean>('autoStart', false);
  ...
  override onUpdate(context: LogicExecutionContext): void {
    ...
    const running = this.getState<boolean>('running', false);
    if (!running) return;

    this.elapsed += context.deltaTime;
    const duration = this.getConfig<number>('duration', 10);
  ...
  checkAndConsumeComplete(): boolean {
    const shouldComplete = this.getState<boolean>('shouldComplete', false);
    if (shouldComplete) {
