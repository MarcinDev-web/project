/**
 * AvatarBuilderPage - Main Avatar Customization Studio
...
} from '../components/avatar-builder-ui';
    ...
    const loadSavedLoadout = async () => {
      ...
      try {
        ...
        if (savedLoadout) {
          ...
          const migratedLoadout = migrationResult.loadout;
          
          // Merge with default loadout to ensure all slots have parts
          // This prevents "brown ball" issue where avatar disappears because
          // saved loadout had fewer slots than expected
          const mergedParts: AvatarLoadout['parts'] = { ...DEFAULT_AVATAR_LOADOUT.parts };
          
          // Only override with saved parts if they have a valid mesh
          for (const slot of AVATAR_SLOTS) {
            const savedPart = migratedLoadout.parts?.[slot];
            if (savedPart && savedPart.mesh) {
              // Check if mesh exists in part library
          ...
          const finalLoadout: AvatarLoadout = {
            version: migratedLoadout.version,
            parts: mergedParts,
          };
    ...
    return (
      <Layout>
        <div className="avatar-builder">
          <div className="builder-loading" style={{ minHeight: '80vh' }}>
            <div className="builder-loading__spinner" />
            <span>Loading Avatar Builder...</span>
          </div>
        </div>
      </Layout>
    );
    ...
    <Layout>
      <div className="avatar-builder">
        ...
        <header className="avatar-builder__header">
          <div className="avatar-builder__header-left">
            <button
              className="avatar-builder__back-btn"
              onClick={() => navigate(-1)}
            >
              ← Back
            </button>
            <h1 className="avatar-builder__title">Avatar Builder</h1>
          </div>

          <div className="avatar-builder__header-actions">
            <button
              className="avatar-builder__history-btn"
              onClick={handleUndo}
              disabled={!canUndo}
              title="Undo (Ctrl+Z)"
            >
            ...
            <button
              className={`builder-btn builder-btn--secondary ${showColorPicker ? 'builder-btn--active' : ''}`}
              onClick={() => {
            ...
            <button
              className={`builder-btn builder-btn--secondary ${showCommunityPresets ? 'builder-btn--active' : ''}`}
              onClick={() => {
        ...
        {/* Main content */}
        <div className="avatar-builder__main">
          ...
          <div className="avatar-builder__viewport-container">
            ...
            {/* Hover indicator */}
            {hoverLoadout && (
              <div className="avatar-builder__hover-indicator">
                <span>👁️ Preview</span>
              </div>
            )}
          </div>
          ...
          {showColorPicker ? (
            <div className="avatar-builder__color-panel">
              <ColorPickerAdvanced
                color={currentColor}
