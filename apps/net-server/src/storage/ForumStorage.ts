...
export class ForumStorage {
  ...
  async createCategory(category: Omit<ForumCategory, 'threadCount' | 'postCount'>): Promise<ForumCategory> {
    const categories = await this.readCategories();
  ...
  async getForumStats(): Promise<{
    ...
    }).sort((a, b) => b.threadCount - a.threadCount).slice(0, 5);
  ...
  async getThreads(categoryId?: string, sortBy: 'hot' | 'new' | 'top' = 'hot'): Promise<ForumThread[]> {
    const threads = await this.readThreads();
  ...
  async createThread(thread: Omit<ForumThread, 'id' | 'postCount' | 'lastPostAt' | 'lastPostBy' | 'createdAt' | 'updatedAt' | 'reactions' | 'score' | 'upvotes' | 'downvotes'>): Promise<ForumThread> {
    const threads = await this.readThreads();
    const now = Date.now();
  ...
  async updateThread(id: string, updates: Partial<ForumThread>, _userId: string): Promise<ForumThread | null> {
    const threads = await this.readThreads();
  ...
  async getPosts(threadId: string, sortBy: 'new' | 'top' = 'new'): Promise<ForumPost[]> {
    const posts = await this.readPosts();
    let filtered = posts.filter(p => p.threadId === threadId);
  ...
  async updatePost(id: string, updates: Partial<ForumPost>, userId: string): Promise<ForumPost | null> {
    const posts = await this.readPosts();
  ...
  // Reactions
  async addReaction(threadId: string | null, postId: string | null, emoji: string, userId: string): Promise<boolean> {
    if (!threadId && !postId) {
      return false;
    }
    ...
    if (threadId) {
      ...
      thread.reactions = thread.reactions.filter(r => !(r.userId === userId && r.emoji === emoji));
  ...
  async removeReaction(threadId: string | null, postId: string | null, emoji: string, userId: string): Promise<boolean> {
    if (!threadId && !postId) {
      return false;
    }
  ...
  async voteThread(threadId: string, userId: string, vote: 'up' | 'down'): Promise<{ score: number; upvotes: number; downvotes: number }> {
  ...
  async removeThreadVote(threadId: string, userId: string): Promise<{ score: number; upvotes: number; downvotes: number }> {
    const votes = await this.readThreadVotes();
    const threads = await this.readThreads();
    const thread = threads.find(t => t.id === threadId);
  ...
  async votePost(postId: string, userId: string, vote: 'up' | 'down'): Promise<{ score: number; upvotes: number; downvotes: number }> {
    const votes = await this.readPostVotes();
  ...
  async removePostVote(postId: string, userId: string): Promise<{ score: number; upvotes: number; downvotes: number }> {
    const votes = await this.readPostVotes();
    const filteredVotes = votes.filter(v => !(v.postId === postId && v.userId === userId));
    await this.writePostVotes(filteredVotes);
